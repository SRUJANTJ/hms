...
export default function AdminAttendance() {
  ...
  return (
    <PortalLayout role="admin" user={user} links={ADMIN_LINKS} title="Attendance">
                  ...
                  <div className="flex gap-2">
                    {["Present", "Absent", "Leave"].map((s) => (
                      <button
                        ...
                        className={`btn text-xs py-1 ${r.status === s ? (s === "Present" ? "bg-emerald-600 text-white" : s === "Absent" ? "bg-red-600 text-white" : "bg-amber-500 text-white") : "btn-outline"}`}
                      >
                        {s}
                      </button>
                    ))}
